...
class KNN:
    ...
    def Predict(self, testFeatures):
        ...
        for i in range(testFeatures.shape[0]):
            ...
            neighbors = self.trainY[first_k_neighbor_index].astype(int)
            # count the answers, give the final answer
            # print(neighbors, 'n')
            prediction = np.argmax( np.bincount(neighbors.reshape(len(neighbors))) ) # originally just np.bincount(neighbors)
